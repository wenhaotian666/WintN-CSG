...
from coop import CustomCLIP,CoOp
...
def zeroshot_classifier(classnames, templates, model):
    with torch.no_grad():
        ...
        for classname in classnames:
            texts = [template for template in templates] #列表的每个元素就是'a clean origami {类名}.'
    ...
    return zeroshot_weights.t()#返回转置后的权重张量，这样每一列对应一个类别。
...
def perform(process_id,bg_text_features,dataset_list, args, model, cam): #0，datalist=trainlist，
    #准备gpu设备
    ...
    for im_idx, im in enumerate(tqdm(databin)):
        ...
        for _ in range(30):
            for image in ms_imgs:
                ...
                image_features, attn_weight_list = model.encode_image(image, h, w)#得到图像特征和注意力列表(577,1,768)
                ...
                image_features_change=image_features.permute(1, 0, 2)
                vit=VisionTransformer(224, 16, width=768, layers=12,heads=8, output_dim=512)
                image_features_change=vit.ln_post(image_features_change.to("cpu"))
                image_features_change = image_features_change[:, 0, :]
                image_features_change=image_features_change.float() @ vit.proj  #（1，512）

                # coop CustomCLIP forward
                prompts = customclip.prompt_learner()
                tokenized_prompts = customclip.tokenized_prompts
                text_features = customclip.text_encoder(prompts, tokenized_prompts)
                image_features_change = image_features_change / image_features_change.norm(dim=-1, keepdim=True)
                image_features_change = image_features_change.to(device)
                text_features = text_features / text_features.norm(dim=-1, keepdim=True)
                logit_scale = torch.nn.Parameter(torch.ones([]) * np.log(1 / 0.07)).exp()

                logits = logit_scale * image_features_change @ text_features.t() # 计算图像和文本特征之间的相似度

                gt = torch.zeros(1, 20).float().to("cuda")
                gt[0,label_id_list] = 1


                loss = torch.nn.functional.cross_entropy(logits, gt).to("cuda")
                # loss = criterion().to("cuda")
                ...
                fg_features_temp = text_features[label_id_list].to(device_id)
                ...
                text_features_temp.requires_grad=True
            ...
            for idx, label in enumerate(label_list):
                ...
                if idx == 0:
                    ...
                    attn_weight = torch.stack(attn_weight, dim=0)[-8:]#表示取堆叠结果的最后 8 个张量，这步没懂为什么
                ...
                aff_mask = aff_mask.view(1,grayscale_cam.shape[0] * grayscale_cam.shape[1]) #Bc矩阵向量化
                aff_mat = attn_weight
                ...
                trans_mat = trans_mat * aff_mask #At*Bc
...
if __name__ == "__main__":
    ...
    # config_path="F:\Studyfiles\CLIP-ES-main/vit_b16.yaml"
    # cfg = load_config(config_path)

    # CoOp
    customclip = CustomCLIP(classnames, model).to(device)
    ...
    customclip.to(device)

    # NOTE: only give prompt_learner to the optimizer
    optimizer = optim.SGD(customclip.prompt_learner.parameters(), 0.002,0.9)
    scheduler = build_lr_scheduler(optimizer)


    bg_text_features = zeroshot_classifier(BACKGROUND_CATEGORY, ['a clean origami {}.'], model)#['a rendering of a weird {}.'], model)
    # fg_text_features = zeroshot_classifier(new_class_names, fg_text_features, model)#['a rendering of a weird {}.'], model)
    ...
    if args.num_workers == 1:
        perform(0, bg_text_features,dataset_list, args, model, cam)
    else:
        multiprocessing.spawn(perform, nprocs=args.num_workers,
                              args=(dataset_list, args, model,  cam))