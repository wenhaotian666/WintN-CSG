...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, H, W):
        ...
        self.positional_embedding_new = upsample_pos_emb(self.positional_embedding, (H//16,W//16)) #(1,3,288,512)
        x = self.conv1(x)  # shape = [*, width, grid, grid] (1,768,18,32)
        x = x.reshape(x.shape[0], x.shape[1], -1)  # shape = [*, width, grid ** 2] (1,768,576)
        x = x.permute(0, 2, 1)  # shape = [*, grid ** 2, width](1,576,768)
        x = torch.cat([self.class_embedding.to(x.dtype) + torch.zeros(x.shape[0], 1, x.shape[-1], dtype=x.dtype, device=x.device), x], dim=1)  # shape = [*, grid ** 2 + 1, width] (1,577,768)
        x = x + self.positional_embedding_new.to(x.dtype)#(1,577,768)
        x = self.ln_pre(x)
...
class CLIP(nn.Module):
    ...
    def forward_last_layer(self, image_features, text_features):
        ...
        logits_per_image = logit_scale * image_features @ text_features.t() ##image_features.float()
